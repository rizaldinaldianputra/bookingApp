...
const renderRecommendedItem = ({ item }: { item: Kamar }) => {
  ...
  console.log('Index:', item.id, 'Nama:', item.nama_kamar, 'kosaan id', item.kos.id);
...
const FilterModal = ({ isVisible, onClose, onApplyFilter, initialFilters }: FilterModalProps) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const lokasiData: LokasiResponse = await getLokasi(); // Tanpa parameter
...
export default function SearchByLokasiScreen() {
  const { name } = useLocalSearchParams<{ name: string }>();
  const [isFilterModalVisible, setFilterModalVisible] = useState(false);
  const [activeFilters, setActiveFilters] = useState<FilterState>({
    location: name || '', // Set initial location from params
    time: '',
    gender: '',
    facilities: [],
    minPrice: 0,
  ...
  const fetchKosData = useCallback(
    async (page: number, filters: FilterState, isNewSearch: boolean = false) => {
      ...
      try {
        ...
        if (filters.checkInDate) params.start_date = filters.checkInDate;
        if (filters.checkOutDate) params.end_date = filters.checkOutDate;
