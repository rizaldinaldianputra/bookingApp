...
// --- PembeliaList Component ---
export default function PembeliaList() {
  ...
  const { data, loading, error, refetch } = useTransaksiProduct(user?.id?.toString() || '');
  const router = useRouter();
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      (item: any) =>
        item.no_order.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.tanggal.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.harga.toString().toLowerCase().includes(searchTerm.toLowerCase()),
    );
  }, [data, searchTerm]);
  ...
  const renderItem = ({ item }: { item: any }) => (
    <View style={styles.row}>
      <Text style={styles.cell}>#{item.no_order}</Text>
      <Text style={styles.cell}>
        {new Date(item.tanggal_transaksi).toLocaleDateString('id-ID')}
      </Text>
      <Text style={styles.cell}>Rp{Number(item.subtotal).toLocaleString('id-ID')},-</Text>
      <TouchableOpacity
    ...
    <SafeAreaView style={styles.flexContainer}>
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.row, styles.header]}>
          <Text style={[styles.cell, styles.headerText]}>No Order</Text>
